...
//$find_events = mysqli_query($db, "SELECT * FROM `events` WHERE LCASE(`title`)= '$title' ORDER BY `start-date`");
$find_events = mysqli_query($db, "SELECT * FROM `events` WHERE MATCH (`title`) AGAINST ('$title')");
//$event = mysqli_fetch_assoc($find_events);
...
?>
    ...
    <div class="event-holder" data-id="<?=$event['id']?>">
        <div class="event-box">
            <article class="event">
